<script>
    var ProductManager = new Vue({
        ...
        data: {
            dialogVisible: false,
            savingProduct: false,
            cloningProduct: false,  // 指示是否正在克隆课程
            currentTab: 'basic',
            // 课程的颜色
            ...
            productImages: [],
            // 课程的额外信息
            editExistProductOption: false,  // 表示是否处于课程option编辑的模式
            idOfEditingProductOptionIndex: null, // 保存正在被编辑的课程option的索引值
            ...
            product: {
                ...
                attribute_set_id: {{ $product->attribute_set_id ? $product->attribute_set_id : \App\Models\Utils\ProductType::$BASIC_PRODUCT_ATTRIBUTE_SET }},  // 课程所属的属性集合ID
            ...
            rules: {
                name: [
                    { required: true, message: 'Course Name Required', trigger: 'blur' }
                ],
                sku: [
                    { required: true, message: 'Course SKU Required', trigger: 'blur' }
                ],
                stock: [
                ...
                position: [
                    { required: true, message: 'Course\'s position is Required', trigger: 'blur' }
                ]
            },
        ...
        methods: {
            ...
            switchOnAddGroupProductForm: function(){
                ...
                if(!this.product.id && this.product.is_group_product === true){
                    ...
                    this.$message.error('请您先保存当前的课程, 然后才能继续添加课程组合!');
                }
            },
            ...
            confirmToAddThisGroupProduct: function(){
                ...
                if(this.tempGroupProductQuantity < 1){
                    // 提示, 课程数量至少为1
                    this._notify('error','Error','组合课程中的子课程数量至少为1个!');
                }else{
                    var that = this;
                    var p = {
                ...
                axios.post('/api/products/rm-group-product',{gpid:groupProductId})
                    .then(function(res){
                        if(res.data.error_no === 100){
                            ...
                            that._notify('success', 'Done', '您所选择的子课程已经被删除!');
                        }else{
                            // 添加失败
                            that._notify('error','Error','系统繁忙,请稍候再试!');
                        }
            ...
            _loadProductImages: function(){
                ...
                axios.post(
                    ...
                    if(res.data.error_no == 100){
                        ...
                        // 失败
                        that._notify('info','Notice','课程还没有添加过图片!');
                    }
                });
            },
            ...
            _cloneProduct: function(){
                if(this.categories.length == 0){
                    this._notify('error','请给课程至少指定一个分类');
                }else{
                    var that = this;
                    ...
                    // 由于使用了 vuejs-editor, 需要单独通过下面的方式获取课程的description最新值
                    ...
                    axios.post(
                            '<?php echo url('api/products/clone') ?>',
                            {
                                ...
                                productAttributeData: paData // 和课程属性相关的值
                            }
                    ).then(function(res){
                        if(res.data.error_no == 100){
                            // 成功 从新加载一下
                            that._notify('success','DONE!','Course Cloned!');
                            window.location.href = '<?php echo url('backend/products/edit'); ?>/' + res.data.msg;
                        }else{
                            // 失败
                            that._notify('error','Error','Can not clone course, please try later!');
                        }
            ...
            _saveProduct: function(){
                ...
                if(this.categories.length == 0){
                    ...
                    axios.post(
                        ...
                        if(res.data.error_no == 100){
                            // 成功 从新加载一下
                            that._notify('success','DONE!','Course Saved!');
                            window.location.href = '/backend/products/edit/' + res.data.msg;
                        }else{
                            // 失败
                            that._notify('error','Error','Can not save course, please try later!');
                        }
...
//                console.log(file, fileList);
            ...
            /**
             * 显示对话框, 询问是否需要删除给定index的课程option
             */
            ...
            _afterOptionAdded: function () {
                // 当课程的 Option 添加完成之后, 重置整个的 Option 表单
                this.hideProductOptionItemForm = true;
