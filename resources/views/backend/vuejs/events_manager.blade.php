<script type="application/ecmascript">
    function isUniqueUri(rule, value, callback){
        if (!value) {
            return callback(new Error('Event URI is required'));
        } else {
            // 验证其唯一性
            axios.post('/api/events/is-uri-unique',{uri:value,action:'{{ isset($actionName) ? $actionName : 'new' }}', id:'{{ $page->id }}'})
                .then(function(res){
                    if(res.data.error_no == 100 && res.data.msg == 'OK'){
                        // 表示为给定URI是唯一的
                        callback();
                    }else {
                        callback(new Error('Event URI must be unique, "' + value + '" has been used!'));
                    }
                });
        }
    }
    ...
    var PagesManager = new Vue({
        ...
        data: {
            ...
            currentPage: {
                ...
                attendees_limit:'{{ $page->attendees_limit }}',
                start:'{{ $page->start }}',
                end:'{{ $page->end }}',
            ...
            _savePage: function(){
                ...
                axios.post(
                    ...
                    if(res.data.error_no == 100){
                        ...
                        // 失败
                        that._notify('error','Error','Can not save event, please try later!');
                    }
